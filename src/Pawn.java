    ...
    public ArrayList<Square> getValidMoves() {
        ...
        try {
            if (this.color.equals("white")) {
                if (this.pos_Y == 6) { // checks to see if the pawn is on its' inital row which then allows the pawn to move forward two spaces
                ...
                if (this.pos_Y > 0) { // checks the left diagonal for a white piece
                    Square check = ChessBoard.squares[this.pos_X-1][this.pos_Y+1]; //Don't need to worry about out of bounds on x because it will promote.
            ...
            //EN CROISSANT || The compound chained if statments are a bit hard to understand, but I still think they look a bit better than a stack of nested statements.
            ...
            if(this.pos_X>0 && ChessBoard.squares[this.pos_X-1][this.pos_Y].isOccupied() && ChessBoard.squares[this.pos_X-1][this.pos_Y].pieceOnSquare().name.equals("Pawn")){
                ...
                Pawn otherPawn = (Pawn)ChessBoard.squares[this.pos_X-1][this.pos_Y].pieceOnSquare(); //We already checked that it was a pawn above.
                // Is it a pawn move    |  Was it the neighboring pawn                                                         | Did it double move (vs moving 1 square twice) 
                if(lastMove.length()==2 && ChessStrings.xLookup.indexOf(Integer.parseInt(lastMove.substring(1)))==this.pos_X-1 && otherPawn.doubleMoved)
                    moves.add(ChessBoard.squares[this.pos_X-1][this.pos_Y]);     //Fun fact if you're looking at this in VS Code: That 'beginIndex:' isn't properly monospaced at the end, which is why the comment doesn't line up.
            }
